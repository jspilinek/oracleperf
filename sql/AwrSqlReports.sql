DEF filename = 'debug_AwrSqlReports-Driver.sql';
...
SPOOL html/debug_AwrSqlReports-Driver.sql REPLACE
PRO REM =======================================================================
PRO REM This file is automatically generated by sql/AwrSqlReports.sql
PRO REM
PRO REM Purpose: 
PRO REM   Driver script used to generate AWR SQL reports if T or D was entered for license pack.
PRO REM
PRO REM Notes:
...
--Average Elapsed Time:
    FOR cur_snap IN (select dbid, instance_number, sql_id, min_snap, max_snap, avg_time FROM (
        WITH 
          sqlavg AS (SELECT ss.dbid,
            ss.instance_number,
            ss.sql_id, 
          ...
          GROUP BY ss.dbid, ss.instance_number, ss.sql_id
          ORDER BY avg_time DESC),
          
        snap_range AS (
        SELECT ss.dbid, ss.instance_number, ss.sql_id, min(snap.snap_id)-1 min_snap, max(snap.snap_id) max_snap
          FROM dba_hist_sqlstat ss, dba_hist_snapshot snap, dba_hist_sqltext sqltext
        WHERE ss.snap_id not in (select max(snap_id) from dba_hist_sqlstat)
          and ss.snap_id = snap.snap_id
          and ss.sql_id = sqltext.sql_id
          ...
          and ss.parsing_schema_name not in 
            ...
            and sqltext.sql_text not like '%/* DS_SVC%'
            and sqltext.sql_text not like 'BEGIN DBMS_STATS%'
        GROUP BY ss.dbid, ss.instance_number, ss.sql_id, snap.startup_time
        ORDER BY ss.sql_id
        ),

        max_snap_range AS(
        SELECT dbid, instance_number, sql_id, MAX(min_snap) min_snap, MAX(max_snap) max_snap
        FROM snap_range
        GROUP BY dbid, instance_number, sql_id
        )

        SELECT * FROM (
        SELECT sqlavg.dbid, sqlavg.instance_number, sqlavg.sql_id, max_snap_range.min_snap, max_snap_range.max_snap, sqlavg.avg_time
        FROM sqlavg, max_snap_range
        WHERE sqlavg.sql_id = max_snap_range.sql_id
          AND sqlavg.max_snap_id = max_snap_range.max_snap
          AND sqlavg.dbid = max_snap_range.dbid
          AND sqlavg.instance_number = max_snap_range.instance_number
    ...
    LOOP
        ...
        dbms_output.put_line('SELECT output FROM TABLE(DBMS_WORKLOAD_REPOSITORY.awr_sql_report_html('||cur_snap.dbid||','||cur_snap.instance_number||','||cur_snap.min_snap||','||cur_snap.max_snap||','''||cur_snap.sql_id||'''));');
    ...
    END LOOP;
        ...
        dbms_output.put_line('SPOOL OFF');
--Average Buffer Gets
    FOR cur_snap IN (select dbid, instance_number, sql_id, min_snap, max_snap, avg_gets FROM (
        WITH 
          sqlavg AS (SELECT ss.dbid,
            ss.instance_number,
            ss.sql_id, 
        ...
        SELECT * FROM (
        SELECT sqlavg.dbid, sqlavg.instance_number, sqlavg.sql_id, max_snap_range.min_snap, max_snap_range.max_snap, sqlavg.avg_gets
        FROM sqlavg, max_snap_range
        WHERE sqlavg.sql_id = max_snap_range.sql_id
          AND sqlavg.max_snap_id = max_snap_range.max_snap
          AND sqlavg.dbid = max_snap_range.dbid
...
/
SPOOL OFF
@@html/debug_AwrSqlReports-Driver.sql
